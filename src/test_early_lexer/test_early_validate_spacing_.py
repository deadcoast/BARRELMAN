...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "::  :: invalid spacing",
            4,
            "[Line 4] ERROR: Extra spacing on nested '::' block.",
        ),
        (
            "some text :^: invalid position",
            5,
            "[Line 5] ERROR: ':^:' must be at the start of the line or have a single space before it.",
        ),
...
class TestBarrelmanLexerValidateSpacing:
    ...
    def test_edge_cases(self):
        ...
        lexer = BarrelmanLexer("")
        assert (
            lexer.validate_spacing("::  :: invalid spacing", 4)
            == "[Line 4] ERROR: Extra spacing on nested '::' block."
        )
        assert (
            lexer.validate_spacing("some text :^: invalid position", 5)
            == "[Line 5] ERROR: ':^:' must be at the start of the line or have a single space before it."
        )
        assert (
            lexer.validate_spacing("::", 6) is None
        )  # Edge case: minimal valid '::' line
        assert (
            lexer.validate_spacing(":^:", 7) is None
        )  # Edge case: minimal valid ':^:' line
        assert (
            lexer.validate_spacing("", 8) is None
        )  # Edge case: empty line        assert lexer.validate_spacing(":^:", 7) is None  # Edge case: minimal valid ':^:' line
